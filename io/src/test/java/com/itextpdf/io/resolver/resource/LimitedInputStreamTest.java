...
public class LimitedInputStreamTest extends ExtendedITextTest {
    ...
    public void readingByteWithLimitOfOneLessThenFileSizeTest() throws IOException {
        try (InputStream stream = new LimitedInputStream(new TestStreamGenerator().openStream(), 88);) {
            for (int i = 0; i < 88; i++) {
    ...
    public void readingByteArrayWithSmallBufferTest() throws IOException {
        try (InputStream stream = new LimitedInputStream(new TestStreamGenerator().openStream(), 89);) {
    ...
    public void byteArrayOverwritingTest() throws IOException {
        try (InputStream stream = new LimitedInputStream(new TestStreamGenerator().openStream(), 90);) {
            byte[] bytes = new byte[100];
            bytes[89] = 13;
    ...
    public void readingByteWithZeroLimitTest() throws IOException {
        try (LimitedInputStream stream = new LimitedInputStream(new ByteArrayInputStream(new byte[1]), 0);) {
            Assertions.assertThrows(ReadingByteLimitException.class, () -> stream.read());
        }
    }
    ...
    public void readingEmptyByteWithZeroLimitTest() throws IOException {
        try (LimitedInputStream stream = new LimitedInputStream(new ByteArrayInputStream(new byte[0]), 0);) {
    ...
    public void illegalReadingByteLimitValueTest() {
        ...
        Assertions.assertEquals(IoExceptionMessageConstant.READING_BYTE_LIMIT_MUST_NOT_BE_LESS_ZERO, e.getMessage());
    }

    static class TestStreamGenerator {
        String data = "body {\n" +
