 ...
 */
public class NameConstraintsBC extends ASN1EncodableBC implements INameConstraints {
    /**
     * Creates new wrapper instance for {@link NameConstraints}.
     *
     * @param nameConstraints {@link NameConstraints} to be wrapped
     */
    public NameConstraintsBC(NameConstraints nameConstraints) {
    ...
    public IGeneralSubtree[] getPermittedSubtrees() {
        ...
        if (permittedSubtress == null) {
            ...
            return Arrays.stream(permittedSubtress).map(subtree ->
                    new GeneralSubtreeBC(subtree)).collect(Collectors.toList()).toArray(new GeneralSubtreeBC[0]);
        }
    }
    ...
    public IGeneralSubtree[] getExcludedSubtrees() {
        ...
        if (excludedSubtrees == null) {
            ...
            return Arrays.stream(excludedSubtrees).map(subtree ->
                    new GeneralSubtreeBC(subtree)).collect(Collectors.toList()).toArray(new GeneralSubtreeBC[0]);
        }
    }
}
