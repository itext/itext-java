...
public class DynamicMarginsFootnoteLayoutResultTest extends ExtendedITextTest {
    ...
    public void hugeTopMarginPartialTest() {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(new ByteArrayOutputStream()));
            ...
            Assertions.assertEquals(LayoutResult.PARTIAL, status,
                    "Tall element should be split (PARTIAL) when a large dynamic top margin "
    ...
    public void hugeTopAndBottomMarginsPartialTest() {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(new ByteArrayOutputStream()));
            ...
            Assertions.assertEquals(LayoutResult.PARTIAL, status,
                    "Tall element should be split (PARTIAL) when both dynamic top and bottom "
                            + "margins are large but a small usable strip remains");
        }
    }

    @Test
    public void dynamicMarginsEvenPagesRenderTest()
    ...
    @Test
    public void dynamicMarginsOddPagesRenderTest()
            throws IOException, InterruptedException {
    ...
    @Test
    public void dynamicMarginsSectionBreakRenderTest()
            throws IOException, InterruptedException {
        ...
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(outFileName));
            ...
            document.setPageMargins(
                    pageNum -> pageNum % 2 == 0,
                    PageMarginsTestUtil.getMarginBoxes(largeTop, 0, 0, 0));
    ...
    @Test
    public void perPageDynamicMarginsRenderTest()
            throws IOException, InterruptedException {
        ...
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(outFileName));
            ...
            document.setPageMargins(pageNum -> {
                ...
                list.add(new PageMarginContent(MarginBoxName.TOP,
                        new Div()
                                .add(new Paragraph("Header p." + pageNum))
    ...
    public void largeFootnotePartialTest() {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(new ByteArrayOutputStream()));
            ...
            Assertions.assertEquals(LayoutResult.PARTIAL, status,
                    "Tall element should be split (PARTIAL) when a large footnote margin "
                            + "leaves less than half the page for content");
        }
    }

    @Test
    public void largeFootnotePartialRenderTest()
            throws IOException, InterruptedException {
    ...
    @Test
    public void largeFootnoteAreaBreakRenderTest()
            throws IOException, InterruptedException {
    ...
    public void largeFootnoteAndHeaderPartialTest() {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(new ByteArrayOutputStream()));
            ...
            Assertions.assertEquals(LayoutResult.PARTIAL, status,
                    "Tall element should be split (PARTIAL) when both header and large "
                            + "footnote margins leave only a small usable band");
        }
    }

    @Test
    public void dynamicFootnoteMarginsEvenPagesRenderTest()
            throws IOException, InterruptedException {
    ...
    public void extremeTopMarginNothingTest() {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(new ByteArrayOutputStream()));
            ...
            Assertions.assertEquals(LayoutResult.NOTHING, status,
                    "Element should return NOTHING when a dynamic top margin occupies "
    ...
    public void extremeTopAndBottomMarginsNothingTest() {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(new ByteArrayOutputStream()));
            ...
            Assertions.assertEquals(LayoutResult.NOTHING, status,
                    "Element should return NOTHING when combined dynamic top and bottom margins "
    ...
    public void extremeFootnoteNothingTest() {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(new ByteArrayOutputStream()));
            ...
            Assertions.assertEquals(LayoutResult.NOTHING, status,
                    "Short element with explicit height should return NOTHING when a huge "
    ...
    public void hugeDynamicMarginForcedPlacementRenderTest()
            throws IOException, InterruptedException {
        ...
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(outFileName));
            ...
            Div forced = new Div()
                    .add(new Paragraph("FORCED — huge dynamic top margin, almost no space."))
    ...
    @Test
    @LogMessages(messages = @LogMessage(messageTemplate = LayoutLogMessageConstant.ELEMENT_DOES_NOT_FIT_AREA))
    public void hugeFootnoteForcedPlacementRenderTest()
            throws IOException, InterruptedException {
        ...
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(outFileName));
            ...
            Div forced = new Div()
                    .add(new Paragraph("FORCED — huge footnote margin, almost no space."))
                    .setBackgroundColor(new DeviceRgb(255, 100, 100));
            document.add(forced);
        }
    ...
    @Test
    @LogMessages(messages = @LogMessage(messageTemplate = LayoutLogMessageConstant.ELEMENT_DOES_NOT_FIT_AREA))
    public void extremeMarginsRecoveryViaSectionBreakRenderTest()
            throws IOException, InterruptedException {
        ...
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(outFileName));
            ...
            Div forced = new Div()
                    .add(new Paragraph("PAGE 1 — extreme dynamic margins (FORCED)."))
                    .setBackgroundColor(new DeviceRgb(255, 100, 100));
            document.add(forced);

            document.add(new SectionBreak(PageMarginsTestUtil.getMarginBoxes(80, 80, 0, 0)));
            document.add(new Div()
                    .add(new Paragraph("PAGE 2 — normal margins, content fits fully."))
    ...
    public void extremeFootnoteMarginsRecoveryRenderTest()
            throws IOException, InterruptedException {
        ...
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(outFileName));
            ...
            Div forced = new Div()
                    .add(new Paragraph("PAGE 1 — extreme footnote margin (FORCED)."))
                    .setBackgroundColor(new DeviceRgb(255, 100, 100));
            document.add(forced);

            document.add(new AreaBreak());
            Div also = new Div()
                    .add(new Paragraph("PAGE 2 — same extreme footnote margin (FORCED)."))
                    .setBackgroundColor(new DeviceRgb(255, 180, 80));
            document.add(also);

            document.add(new SectionBreak(PageMarginsTestUtil.getMarginBoxes(60, 60, 0, 0)));
            document.add(new Div()
                    .add(new Paragraph("PAGE 3 — normal margins, content fits."))
                    .setBackgroundColor(new DeviceRgb(65, 151, 29)));
        }
    ...
    public void dynamicMarginStatusProgressionTest() {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(new ByteArrayOutputStream()));
            ...
            Assertions.assertEquals(LayoutResult.FULL, fullStatus,
                    "Expected FULL with small dynamic top margin");
    ...
    public void footnoteMarginStatusProgressionTest() {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(new ByteArrayOutputStream()));
            ...
            Assertions.assertEquals(LayoutResult.FULL, fullStatus,
                    "Expected FULL with a small footnote margin");
    ...
    public void combinedDynamicAndFootnoteMarginStatusProgressionTest() {
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(new ByteArrayOutputStream()));
            ...
            Assertions.assertEquals(LayoutResult.FULL, fullStatus,
                    "Expected FULL when both margins are small");
    ...
    public void hugeTopDynamicMarginWithFootnoteAnchorThrowsTest() {
        float largeTop = A4_HEIGHT * 0.55f;
    ...
    @Test
    //TODO DEVSIX-9981: Adapt test after fix
    public void hugeBottomDynamicMarginWithFootnoteAnchorThrowsTest() {
    ...
    @Test
    public void hugeTopAndBottomMarginsWithFootnoteAnchorRenderTest()
            throws IOException, InterruptedException {
        ...
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(outFileName));
            ...
            float top    = A4_HEIGHT * 0.35f;
            float bottom = A4_HEIGHT * 0.35f;
            document.setPageMargins(pageNum -> true,
                    PageMarginsTestUtil.getMarginBoxes(top, bottom, 0, 0));

            Footnote footnote =
                    new Footnote(TestResourceUtil.getByronStanza());
            footnote.setBorder(new DashedBorder(
                    ColorConstants.YELLOW, 3));

            FootnoteAnchor anchor =
                    new FootnoteAnchor("[1]", footnote);
            Paragraph p = new Paragraph(TestResourceUtil.getByronStanza())
                    .add(anchor)
                    .add(TestResourceUtil.getByronStanza());

            document.add(new Div().add(p)
                    .setBorder(new SolidBorder(
                            ColorConstants.GREEN, 2)));
        }
