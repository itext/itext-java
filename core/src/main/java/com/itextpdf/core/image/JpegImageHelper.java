    ...
    private static void updatePdfStream(PdfStream pdfStream, Image image) {
        ...
        switch (image.getColorSpace()) {
            ...
            case 3:
                ...
                break;
            default:
                pdfStream.put(PdfName.ColorSpace, PdfName.DeviceCMYK);
                if (image.isInverted()) {
                    pdfStream.put(PdfName.Decode, new PdfArray(new float[]{1, 0, 1, 0, 1, 0, 1, 0}));
                }
        }
        pdfStream.put(PdfName.BitsPerComponent, new PdfNumber(8));
        if (image.getData() != null) {
            byte[] imgBytes = image.getData();
            pdfStream.getOutputStream().assignBytes(imgBytes, imgBytes.length);
        } else {
            InputStream is = null;
            try {
                is = image.getUrl().openStream();
