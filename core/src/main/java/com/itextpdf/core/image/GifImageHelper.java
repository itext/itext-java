    ...
    private static boolean readFrame(boolean skip, GifParameters gif) throws IOException {
        ...
        if (!skip) {
            try {
                ...
                colorspace.add(new PdfString(PdfEncodings.convertToString(gif.m_curr_table, null)));
                gif.additional = new PdfDictionary();
                gif.additional.put(PdfName.ColorSpace, colorspace);
                if (gif.transparency) {
