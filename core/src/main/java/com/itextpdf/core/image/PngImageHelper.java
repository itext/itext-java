...
import com.itextpdf.basics.image.PngImage;
...
public class PngImageHelper {
    ...
    private static void processPng(InputStream is, PngParameters png) throws IOException, PdfException {
        ...
        try {
            ...
            if (!png.palShades && !png.genBWMask && pal0 == 1) {
                png.additional.put(PdfName.Mask, new PdfLiteral(String.format("[%d %d]", palIdx, palIdx)));
            }
            ...
            if (png.imageData != null) {
                ...
                decodeparms.put(PdfName.BitsPerComponent, new PdfNumber(png.bitDepth));
                decodeparms.put(PdfName.Predictor, new PdfNumber(15));
                decodeparms.put(PdfName.Columns, new PdfNumber(png.width));
                decodeparms.put(PdfName.Colors, new PdfNumber((png.colorType == 3 || (png.colorType & 2) == 0) ? 1 : 3));
                png.additional.put(PdfName.DecodeParms, decodeparms);
            }
            if (png.additional.get(PdfName.ColorSpace) == null)
                png.additional.put(PdfName.ColorSpace, getColorspace(png));
            if (png.intent != null)
                png.additional.put(PdfName.Intent, png.intent);
    ...
    private static PdfObject getColorspace(PngParameters png) {
        if (png.iccProfile != null) {
        ...
        if (png.gamma == 1f && !png.hasCHRM) {
            ...
            if ((png.colorType & 2) == 0) {
                ...
                if (png.gamma != 1f) {
                    ...
                    PdfNumber n = new PdfNumber(png.gamma);
                    gm.add(n);
                    gm.add(n);
                    gm.add(n);
        ...
        while (true) {
            ...
            if (IDAT.equals(marker)) {
                    ...
                    case 0:
                        if (len >= 2) {
                            ...
                            if (png.bitDepth == 16)
                                ...
                                png.additional.put(PdfName.Mask, new PdfLiteral(String.format("[%d %d]", gray, gray)));
                        }
                        break;
                    case 2:
                        if (len >= 6) {
                ...
                if (png.colorType == 3) {
                    ...
                    colorspace.add(getColorspace(png));
