...
public class IndicTextExtractionTest extends ExtendedITextTest {
    ...
    @Test
//    @Ignore("Seems to be a problem with decode. Also note that /ActualText is parsed. For pure content stream parsing set useActualText to false")
    public void test01() throws IOException {
        ...
        for (int i = 0; i < regions.length; i++)
            extractionStrategies[i] = listener.attachEventListener(new LocationTextExtractionStrategy().setUseActualText(false), regionFilters[i]);
...
//    @Ignore("Seems to be a problem with decode. Also note that /ActualText is parsed. For pure content stream parsing set useActualText to false")
    public void test02() throws IOException {
        ...
        Assert.assertEquals(text, PdfTextExtractor.getTextFromPage(pdfDocument.getPage(1), new LocationTextExtractionStrategy().setUseActualText(false)));
    }
