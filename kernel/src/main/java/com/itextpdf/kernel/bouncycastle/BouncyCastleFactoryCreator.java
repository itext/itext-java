...
public final class BouncyCastleFactoryCreator {
    ...
    static {
        try {
            ...
            // If BouncyCastleProvider exists in classpath we assume that default bouncy-castle shall be used.
            factory = new BouncyCastleFactory();
        } catch (ClassNotFoundException ignored) {
            try {
                Class.forName("org.bouncycastle.jcajce.provider.BouncyCastleFipsProvider");
                // If there is no BouncyCastleProvider in classpath, but there is BouncyCastleFipsProvider we assume
                // that bouncy-castle-fips shall be used.
                factory = new BouncyCastleFipsFactory();
            } catch (ClassNotFoundException exception) {
                // If there are neither BouncyCastleProvider nor BouncyCastleFipsProvider we assume that bouncy-castle
